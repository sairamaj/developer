...
public class Solution
{
    ...
    public int CountUnivalSubtrees(TreeNode root, int count)
    {
        if (root == null)
        {
            return count;
        ...
        var ret = root.left == null ? true : root.left.val == root.val;
        var ret1 = root.right == null ? true : root.right.val == root.val;
        if (ret && ret1)
        {
            count++;
        }
        ...
        return CountUnivalSubtrees(root.right, count);
    }

    public int CountUnivalSubtrees(TreeNode root)
    {
        return CountUnivalSubtrees(root, 0);
    }
}
